...
def test_non_registry_specs_are_redacted_before_serialization(spec):
    pin = parse_package_json(
        json.dumps({"dependencies": {"demo": spec}}), "package.json"
    )[0]
...
def test_version_ranges_remain_useful():
    pins = parse_package_json(
        '{"dependencies":{"demo":"^1.2.3","exact":"1.2.3"}}', "package.json"
    )
...
@pytest.mark.parametrize(
    "spec,expected",
    [("1.2.3", None), ("=1.2", None), ("=1.2.3", "1.2.3"), ("^1.2.3", None)],
)
def test_cargo_ranges_are_not_exact_pins(spec, expected):
    assert (
        parse_cargo_toml('[dependencies]\ndemo="' + spec + '"', "Cargo.toml")[0].pinned
        == expected
    )


def test_python_extras_do_not_hide_pin():
    pin = parse_requirements(
        'requests[socks]==2.32.3; python_version >= "3.10"', "requirements.txt"
    )[0]
...
def test_direct_reference_cannot_be_used_as_registry_pin():
    pin = parse_requirements(
        "demo @ https://user:SYNTHETIC_PASSWORD@example.test/demo.whl",
        "requirements.txt",
    )[0]
    assert pin.spec == "[redacted]"
    assert pin.to_dict()["registry_lookup"] is False
...
def test_cargo_source_and_rename_are_not_lost():
    pins = parse_cargo_toml(
        '[dependencies]\nalias={package="real",version="=1.2.3"}\nprivate={git="https://SYNTHETIC_TOKEN@example.test/repo",version="=1.2.3"}',
        "Cargo.toml",
    )
...
def test_cargo_lock_git_source_is_not_registry_version():
    pin = parse_cargo_lock(
        '[[package]]\nname="demo"\nversion="1.2.3"\nsource="git+https://SYNTHETIC_TOKEN@example.test/demo"',
        "Cargo.lock",
    )[0]
    assert pin.pinned is None
    assert pin.to_dict()["registry_lookup"] is False
