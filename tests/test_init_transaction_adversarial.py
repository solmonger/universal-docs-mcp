...
def assert_no_generated_temps(root: Path) -> None:
    assert not [
        ...
        if path.name.startswith(
            (".adapter.json.", ".settings.json.", ".install-state.json.")
        )
...
def test_dry_run_is_zero_write_for_every_valid_topology(
    ...
    all_before = {
        p: (p.lstat().st_mode, p.read_bytes())
    ...
    assert {p: v for p, v in after.items() if p.name != "requirements.txt"} == {
        p: v for p, v in all_before.items() if p.name != "requirements.txt"
    }
    assert not (tmp_path / ".universal-docs/backups").exists()


@pytest.mark.parametrize(
    "bad", ["malformed-settings", "conflict", "symlink-parent", "fifo-parent"]
)
def test_invalid_paths_are_zero_write_in_dry_run_and_apply(
    ...
    if bad == "malformed-settings":
        ...
        (tmp_path / ".claude/settings.json").write_text(
            '{"hooks":{"UserPromptSubmit":[{"hooks":[{"type":"command","command":"/x/universal-docs-command-hook --wrong"}]}]}}'
        )
...
def existing_project(root: Path) -> dict[Path, tuple[bytes, int, int]]:
    ...
    return {
        p: (p.read_bytes(), p.lstat().st_mode, p.lstat().st_mtime_ns)
        for p in (
            root / ".universal-docs/adapter.json",
            root / ".claude/settings.json",
            root / ".claude/unrelated.json",
        )
    }


@pytest.mark.parametrize(
    "boundary", ["backup", "adapter", "settings", "state", "cleanup"]
)
def test_each_forward_mutation_boundary_recovers_and_retries(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, boundary: str
) -> None:
    before = existing_project(tmp_path)
    original_backup, original_atomic = (
        product_cli._write_backup,
        product_cli._atomic_write,
    )
    backup_calls = 0
    ...
    for path, (raw, mode, mtime) in before.items():
        assert (path.read_bytes(), path.lstat().st_mode, path.lstat().st_mtime_ns) == (
            raw,
            mode,
            mtime,
        )
    ...
    for path in (
        (tmp_path / ".universal-docs/backups").glob("*.json")
        if (tmp_path / ".universal-docs/backups").exists()
        else []
    ):
...
def test_recovery_failure_is_bounded_and_never_claims_mismatched_state(
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(
            OSError("recovery /Users/operator/private")
        ),
    )
    code, receipt = invoke(tmp_path, apply=True)
    assert code == 1
...
def test_backup_mutation_boundaries_are_injectable(
    ...
    real_mkdir, real_open, real_fdopen, real_fsync = (
        product_cli.Path.mkdir,
        product_cli.os.open,
        product_cli.os.fdopen,
        product_cli.os.fsync,
    )
...
@pytest.mark.parametrize(
    "operation",
    ["atomic_temp", "atomic_fdopen", "atomic_fchmod", "atomic_fsync", "atomic_replace"],
)
def test_atomic_boundaries_preserve_preimage(
    ...
    names = {
        ...
        "state": ".universal-docs/install-state.json",
    }
    target = tmp_path / names[target_name]
    target.parent.mkdir(parents=True, exist_ok=True)
    old = b'{"old":true}\n'
    target.write_bytes(old)
    real_mkstemp, real_fdopen, real_fchmod, real_fsync, real_replace = (
        product_cli.tempfile.mkstemp,
        product_cli.os.fdopen,
        product_cli.os.fchmod,
        product_cli.os.fsync,
        product_cli.os.replace,
    )
...
@pytest.mark.parametrize(
    "target_name", ["adapter.json", "settings.json", "install-state.json"]
)
def test_atomic_chmod_failure_recovers_transaction(
    ...
    ).read_bytes() == b'{"permissions":{"allow":[]}}\n'
    assert_no_generated_temps(tmp_path)
