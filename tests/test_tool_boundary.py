...
@pytest.mark.parametrize('name,args', [
    ...
    ('get_package_docs', {'package': 'demo', 'version': '../other?token=SYNTHETIC_SECRET'}),
    ('get_package_docs', {'package': 'demo', 'version': 'latest'}),
    ('get_package_docs', {'package': 'demo', 'ecosystem': 'python', 'version': '^1.0'}),
...
async def test_execution_failure_sets_protocol_error(monkeypatch):
    ...
    async def broken(*args):
        return [types.TextContent(type='text', text=json.dumps({'found': None, 'error': 'upstream_unavailable'}))]
...
async def test_concurrency_backpressure_is_immediate(monkeypatch):
    ...
    monkeypatch.setattr(server, '_dispatch_tool', stalled)
    tasks = [asyncio.create_task(server.call_tool('cache_stats', {})) for _ in range(server.MAX_CONCURRENT_TOOLS)]
    try:
...
async def test_tool_payload_has_hard_size_limit(monkeypatch):
    import mcp.types as types
    async def huge(*a):
        return [types.TextContent(type='text', text=json.dumps({'content': 'x' * (256 * 1024)}))]
