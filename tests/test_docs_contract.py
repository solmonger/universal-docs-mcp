...
from universal_docs_mcp.compaction import parse_sections, compact
...
async def test_latest_cache_does_not_relabel_old_docs(monkeypatch, tmp_path):
    ...
    async def package(*a, **kw):
        return SimpleNamespace(name='demo', ecosystem='python', latest_stable=release[0], license=None, docs_url=None, repository=None)
    async def fetch(*a, **kw):
        requested.append(kw['version'])
        return FetchedDocument('docs for ' + str(kw['version']), 'pypi_description', 'https://pypi.org/example')
...
async def test_targeted_section_obeys_content_budget(monkeypatch):
    ...
    async def load(*a, **kw):
        return (info, 'header', '# Usage\n' + 'x' * 5000, False, {'source': 'test', 'source_url': 'https://example.org'})
    monkeypatch.setattr(server, '_load_document', load)
    result = json.loads((await server._handle_get_docs({'package':'demo', 'section':'usage', 'max_tokens':200}))[0].text)
...
async def test_invalid_budget_never_fetches(monkeypatch):
    ...
    monkeypatch.setattr(server, '_load_document', load)
    result = json.loads((await server._handle_get_docs({'package':'demo','section':'usage','max_tokens':False}))[0].text)
...
async def test_registry_rate_limit_is_not_a_missing_package(monkeypatch):
    ...
    monkeypatch.setattr(registries.httpx, 'AsyncClient', lambda **kw: original(transport=transport, **kw))
