...
def test_long_option_abbreviations_fail_closed_before_plan_selection(
...
def test_long_option_abbreviations_fail_closed_before_init(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args: (_ for _ in ()).throw(
            AssertionError("abbreviated option reached init")
        ),
    )
    base = [
        "init",
...
def test_duplicate_guard_runs_before_parser_for_same_and_different_values(
    tmp_path, monkeypatch
):
    root = str(tmp_path)
    cases = [
...
def test_doctor_drops_non_load_bearing_metadata_when_receipt_overflows(
    tmp_path, monkeypatch
):
    root = tmp_path / "project"
    root.mkdir()
    ...
    assert all(
        set(item) == {"id", "status", "reason", "metadata"}
...
def test_oversized_argv_is_constant_bounded_failure():
    ...
    assert rc == 1
    assert receipt == {
        "schema": "universal-docs.doctor/v1",
        "status": "fail",
...
def _init_root(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, *, both: bool = False
) -> Path:
    root = tmp_path / "project"
...
def _assert_safe(
    rc: int, payload: dict, schema: str, *, reason: str | None = None
) -> None:
    assert rc != 0
...
def test_each_doctor_owner_exception_is_contained(
...
def test_each_remaining_doctor_owner_exception_is_reached_and_contained(
    ...
    if seam == "read":
        ...
        monkeypatch.setattr(
            product_cli, "_installation_executable", lambda name: root / name
        )
        ...
        monkeypatch.setattr(
            ...
            lambda path: type(
                ...
                (),
                {
                    "command": (str(root / "universal-docs-preflight"),),
                    "timeout_ms": 30_000,
                },
            )(),
        )
...
@pytest.mark.parametrize("exception", [OSError, RuntimeError, ValueError])
def test_init_dry_run_owner_exceptions_are_abstentions(
    tmp_path, monkeypatch, seam, exception
):
...
@pytest.mark.parametrize("exception", [OSError, RuntimeError, ValueError])
def test_rollback_owner_exceptions_are_contained(
    tmp_path, monkeypatch, seam, exception
):
...
@pytest.mark.parametrize("exception", [OSError, RuntimeError, ValueError])
def test_init_apply_mutation_fault_uses_transaction_recovery(
...
@pytest.mark.parametrize("exception", [OSError, RuntimeError, ValueError])
def test_doctor_state_read_owner_exception_is_contained(
    tmp_path, monkeypatch, exception
):
    root = tmp_path / "project"
    root.mkdir()
    ...
    monkeypatch.setattr(
        product_cli, "_read_state", lambda root: (_ for _ in ()).throw(exception(_EVIL))
    )
    rc, payload = invoke(["doctor", "--project-root", str(root)])
...
def test_response_too_large_is_before_any_project_mutation(
    ...
    monkeypatch.setattr(
        product_cli.Path, "unlink", lambda *args, **kwargs: mutations.append("unlink")
    )
    monkeypatch.setattr(
        product_cli,
        "_assert_receipt_fits",
...
def test_real_maximum_reachable_success_receipts_fit_and_record_boundary(
    tmp_path, monkeypatch
):
    root = _init_root(tmp_path, monkeypatch, both=True)
    package = "p" * 63
    ...
    assert (
        init_receipt["plan"]["section_ids"] == []
    )  # CLI/planner exposes no section option.
...
def test_recognized_command_routing_hides_huge_root_and_arg_lists(
    tmp_path, monkeypatch, command
):
    root = _init_root(tmp_path, monkeypatch)
    if command == "init":
    ...
    rc, payload = invoke(argv)
    assert rc != 0 and payload["schema"] == getattr(
        product_cli, "_" + command.upper() + "_SCHEMA"
    )
