...
async def test_exact_cache_works_without_registry_and_marks_metadata(
    ...
    refreshed = json.loads(
        (await server.call_tool("get_package_docs", {**args, "force_refresh": True}))[
            0
        ].text
    )
...
async def test_outline_paginates_section_map(monkeypatch):
    async def load(*a, **kw):
        return (
            PackageInfo("demo", "python", "1.0.0", ""),
            "",
            "\n".join("## Section " + str(i) for i in range(250)),
            False,
            {},
        )
    ...
    result = json.loads(
        (
            await server.call_tool(
                "get_docs_outline", {"package": "demo", "offset": 100, "limit": 50}
            )
        )[0].text
    )
...
async def test_cached_alias_cannot_extend_original_doc_ttl(monkeypatch, tmp_path):
    from universal_docs_mcp import cache as cache_module

    now = [1000.0]
    monkeypatch.setattr(cache_module.time, "time", lambda: now[0])
    monkeypatch.setattr(server, "cache", DocsCache(tmp_path, ttl=10))
    ...
    monkeypatch.setattr(server, "fetch_package", package)
    monkeypatch.setattr(server, "fetch_docs_content_with_provenance", docs)
    base = {"package": "demo", "ecosystem": "python"}
    await server.call_tool("get_package_docs", base)
    now[0] = 1009
    await server.call_tool("get_package_docs", {**base, "version": "1.2.3"})
    now[0] = 1011
    result = json.loads(
        (await server.call_tool("get_package_docs", {**base, "version": "1.2.3"}))[
            0
