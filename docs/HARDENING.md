...
- Public tool inputs are validated before I/O. Failures use MCP error signaling and safe structured data; upstream outages are unknown, not absence. The real stdio process must not reflect rejected input in SDK stderr, and the 128 KiB bound must cover the serialized MCP result, including both representations and escaping.
