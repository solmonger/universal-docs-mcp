...
| Cross-version compatibility | Integrated; fresh parent verification pending | Execute a current SDK client against the candidate, recording discover/fallback/negotiation and real calls. Add modern server support only if the measured compatibility requirement warrants it; do not claim July-2026 conformance from fallback. Correct wire errors and preserve cancellation/size/privacy behavior. |
| Shared preflight core | Integrated; fresh parent verification pending | Wire-neutral request/result; explicit freshness policy; selected-version and source/hash/age receipt; bounded relevant context from current allowed package sources; one JSON CLI response and explicit failure exit. A range is never installed-version evidence. |
...
Continue on `feat/cross-harness-fresh-docs`. Package preflight and the current-client fallback gate are integrated; 197 offline tests pass, with live/candidate acceptance still separate. Parent corrected legacy/stale version relabeling, latest-alias replacement, no-match/oversize exit status, no-intent relevance, and concurrent stale-cache pruning using failing regressions. Claude/Codex command hooks and Hermes lifecycle hook are in isolated maker lanes. Next: connect the reviewed official source to the shared receipt path, integrate and verify hooks, then implement the OpenClaw/Pi boundary and independently review the frozen candidate. Preserve the accepted `cbcd35c` live install. Every feature needs behavioral RED→GREEN, a focused commit, and actual consumer proof before claims. Do not install profile-wide hooks, modify other Hermes profiles, publish, or broaden live access without the corresponding approval.
